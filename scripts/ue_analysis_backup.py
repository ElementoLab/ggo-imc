...
for p in ['PANEL_G', 'PANEL_H']:
    ...
    plt.savefig(f"figures/{p}_{uE}_{celltype}_composition.pdf")
...
for cond in ['pathology', 'radio']:
    ...
    for pval_form in ['star', 'sci_notation']:
        imc.pl.plot_mwu(
            density,
            save_dir=f'figures/{p}/{uE}_density/',
            kind = 'box-line',
            pval_form=pval_form
        )
