...
#celltype_map = metadata['cell_type_label_1_0']
...
def get_img_size(cell_mask_file_name):
    ...
    return cell_mask[0] * cell_mask[1] / 1e6
...
for PANEL in ['PANEL_G', 'PANEL_H']:
    IMAGE_FILE_PATTERN = metadata[PANEL]['glob_pattern'][f'IMAGE_FILE_PATTERN']
    MASK_FILE_PATTERN = metadata[PANEL]['glob_pattern'][f'MASK_FILE_PATTERN']
    NUCMASK_FILE_PATTERN = metadata[PANEL]['glob_pattern'][f'NUCMASK_FILE_PATTERN']
    CSV_FILE_PATERN = metadata[PANEL]['glob_pattern'][f'CSV_FILE_PATTERN']

    SINGLE_CELL_FILE = metadata[PANEL]['AnnData']['unlabeled_file_name']

    ROI_AREA_FILE = metadata[PANEL]['image_metadata'][f'ROI_AREA_FILE']
    ROI_MEAN_FILE = metadata[PANEL]['image_metadata'][f'ROI_MEAN_FILE']
    ROI_VAR_FILE = metadata[PANEL]['image_metadata'][f'ROI_VAR_FILE']

    ROI_NUC_FILE = metadata[PANEL]['image_metadata'][f'ROI_NUC_FILE']
    ROI_CYTO_FILE = metadata[PANEL]['image_metadata'][f'ROI_CYTO_FILE']

    ROI_SPILLOVER_FILE = metadata[PANEL]['image_metadata'][f'ROI_SPILLOVER_FILE']
    ...
    '''
    image_files = sorted(
        [x for x in glob(IMAGE_FILE_PATTERN)]
    )
    mask_files = sorted(
        [x for x in glob(MASK_FILE_PATTERN)]
    )
    nucmask_files = sorted(
        [x for x in glob(NUCMASK_FILE_PATTERN)]
    )
    csv_files = sorted(
        [x for x in glob(CSV_FILE_PATERN)]
    )
    ...
    files['Missing Data'] = files.isna().any(axis=1)
    files.to_csv(f'metadata/{PANEL}_file_category_all.csv')
    filtered = files[files.isna().any(axis=1)].index.tolist()
    print(f'Filtering following ROIs due to missing data:\n{filtered}')
    files = files.dropna()
    files.to_csv(f'metadata/{PANEL}_file_category_filtered.csv')
    ...
    if os.path.exists(ROI_MEAN_PATH):
    ...
    if os.path.exists(ROI_VAR_PATH):
    ...
    if os.path.exists(ROI_NUC_PATH) and os.path.exists(ROI_CYTO_PATH):
    ...
    if os.path.exists(ROI_SPILLOVER_PATH):
    ...
    sc.pl.umap(
        ...
        frameon = False,
        show = False,
        title = f'UMAP by samples\nPlotting {len(adata)} cells, {UMAP_FRACTION * 100}% of all cells'
    )
    ...
    nrow, ncol, figsize = get_plot_subfig_param(num_var = adata.shape[1])
    fig, axs = plt.subplots(nrow, ncol, figsize = figsize, dpi = 300)
    ...
    var_names = metadata[f'{panel}_var_celltype_groups']
